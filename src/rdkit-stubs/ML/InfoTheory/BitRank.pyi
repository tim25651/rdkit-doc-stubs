...
IntVector: an object containing integers which supports __getitem__() and__len__(). Examples include lists, tuples, Numeric Arrays, and BitVects.

NOTE: Neither sequences nor IntVectors need to support item assignment.It is perfectly acceptable for them to be read-only, so long as they are
random-access.
"""
from rdkit.ML.InfoTheory import entropy as entropy

def AnalyzeSparseVects(self, bitVects, actVals):
    """
    #DOC
    Arguments
...
def CalcInfoGains(self, bitVects, actVals, nPossibleActs, nPossibleBitVals=2):
    """
...
def FormCounts(self, bitVects, actVals, whichBit, nPossibleActs, nPossibleBitVals=2):
    """
...
def RankBits(self, bitVects, actVals, nPossibleBitVals=2, metricFunc=CalcInfoGains):
    """
...
def SparseRankBits(self, bitVects, actVals, metricFunc=AnalyzeSparseVects):
    """
    Rank a set of bits according to a metric function
    Arguments
